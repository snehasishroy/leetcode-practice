...
public class EliminateMaximumNumberOfMonsters {
    /**
     * Approach: Greedy, Sort the monsters by arrival time. Start firing at the start of every minute (0) and check if you can kill the current monster
     * before the time of firing. If no, stop.
     *
     * Was not able to solve this during the contest :( Reason being, I was not able to understand the problem properly.
     *
    ...
    public int eliminateMaximum(int[] dist, int[] speed) {
        ...
        int timeOfFiring = 0;
        for (int i = 0; i < n; i++) {
            if (time[i] <= timeOfFiring) { //can't kill this monster as it arrives before the time of firing
                return i;
            }
            timeOfFiring++;
        }
